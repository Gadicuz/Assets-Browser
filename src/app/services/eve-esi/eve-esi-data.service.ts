...
import { map, expand, tap, distinct, switchMap, mergeMap, reduce, takeWhile, filter, mapTo, toArray, catchError, ignoreElements } from 'rxjs/operators';

import { EVESSOService } from '../eve-sso/eve-sso.module';
import { EsiService, EsiError, EsiAssetsItem, EsiSystemInfo, EsiStructureInfo, EsiStationInfo, EsiOrder, EsiCharOrder, EsiMail, EsiWalletTransaction } from './eve-esi.module';
...
export class EsiDataService {
  ...
  loadTypeInfo(ids: number[]): Observable<Map<number, EsiDataTypeInfo>> {
    return iif(
      ...
      switchMap(typesInfo => from(this.missedIDs(ids, typesInfo.keys())).pipe(
  ...
  private resolveIDs<T>(ids: [number, number][], type: string, fields: [string, string]): Observable<[number, number][]> {
